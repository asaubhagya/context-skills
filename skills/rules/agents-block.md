...
the Context harness — the `rules` skill installed in this repo's skills
directory. In short:

- Call `usage_guide` once per session before the first write.
...
- Chart exactly one Context map (epic + tickets + `blocks` links) plus a spec;
  raise one `reviewRequest` for both together; wait on `get_events`.
- Work tickets one at a time (or in parallel by dependency graph, one
