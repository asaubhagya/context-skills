...
Exact tool-call shapes. Fill `<…>`; keep every key. Everything is created in
the Shared Space unless the owner named another Space. `description` is
capped at 2048 chars — link the artifacts, never paste them.

Shape of the map (v3, since blog-agent 7):
...
│   └── Blog: <title> (de)   … relates + blocked_by the EN master
...
`list_issues {parent_id}` returns **direct children only**: pieces are
found through their channel parent (`list_issues {parent_id: <Blog
parent>, …}`), never through the epic. Every skill resolves the parents
once per run — `list_issues {parent_id: <epic>, label: "lane:<channel>"}`
— or reads their ids from the epic's `## Structure`.
...
Re-run (existing epic): `list_epics {label: "tenant:<slug>"}` →
`update_epic {id, description}` updates the `## Rules` lines that changed —
never a second epic, never the `## Runs` block (the daily brief owns it).

## 2. Channel parents + standing issues — children of the epic

Create in this order; the Daily Brief issue first. Every entry:
`parent_id: <epic>`, `assignee` empty. Re-run: `list_issues {parent_id:
<epic>, label: "lane:<x>"}` before creating — one hit means it exists.
...
| `Blog` | `["lane:blog", "tenant:<slug>", "gate:none"]` | Parent of every blog publish issue (`channel:blog`, every locale). Open children = the pipeline, done children = what is live. blog-drafter, blog-checker, blog-publisher and daily-brief look pieces up with `list_issues {parent_id: <this id>}`. Never a piece directly under the epic. |
...
create_issues {
  ...
  "issues": [{
    "title": "Blog",
    "description": "Channel parent. Every blog publish issue of tenant <slug> — every locale, new and refresh — is a child of this issue, so the whole channel is visible in one list: open children are the pipeline, done children are live. Skills look pieces up with list_issues {parent_id: <this id>}; nothing content-shaped goes directly under the epic. Slots: <cadence>, <tz>.",
    "labels": ["lane:blog", "tenant:<slug>", "gate:none"]
  }]
}
```

After they exist: `update_epic {id: <epic>, description: <the same
...
then, after every publish issue exists and is `blocked_by` this one:

```json
update_issues { "ids": ["<review ticket id>"], "state": "in_review" }
post_comment { "parent_id": "<review ticket id>", "body": "Map and spec charted — please review both" }
request_review { "parent_id": "<review ticket id>",
...
`locale:de` plus `relates:<en master id>`, `blocked_by: ["<en master
id>"]`, description adds "Cascade: publish after EN master is done".

Instagram: `parent_id: <Instagram parent id>`, `title: "Instagram: <hook>
...
Site page: `parent_id: <Site parent id>`, `title: "Site: <page> (en)"`,
...
with label `relates:<backlog child>`, then `complete_issues {ids: [<backlog
child>], workStats}` after a `post_comment {parent_id: <backlog child>,
body: "Graduated → <new ticket>"}`. The child stays in the Backlog as
history; never delete it, never re-parent it.

## 5. `blocked_by` — second pass, once every id exists

Every publish issue: `update_issues {ids: [<publish issue id>], blocked_by:
["<review ticket id>"]}`. Locale variants additionally include the EN
master's id in `blocked_by`. Parents, Backlog and standing issues are not
