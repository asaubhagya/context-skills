...
Call the Context MCP `start_context`, then follow the `daily-brief` skill exactly: read-only for the last 24 h / next 24 h
(published · awaiting approval · failed/skipped · next 24h with buffer · routine health), then post ONE comment on the
Daily Brief issue with `post_comment {parent_id: "<daily brief issue id>", body}` using the skill's template.
Scope: the pieces are the children of the channel parents — Blog <blog parent id>, Instagram <instagram parent id> (`list_issues {parent_id, includeClosed: true}`, follow nextCursor) —
...
`update_epic {id: "<epic id>", description}` with everything above `## Runs` unchanged and the whole description ≤ 2048 characters (drop the oldest brief lines first).
...
Call Context `start_context` and Blog MCP `usage_guide` + `get_capabilities`, then follow the `blog-drafter` skill exactly:
...
children of the Blog parent first (`list_issues {parent_id: "<blog parent id>", label: "channel:blog", state: "open", ready: true}`, earliest due), else graduate the oldest `stage:topic` child of the Backlog
into a new publish issue under the Blog parent (next free slot, `relates` label, mark the Backlog child done with a "Graduated → <ticket>" comment) — never create a piece directly under the epic;
...
variants (<locale depths, e.g. de full · fr reduced>) as `relates` children of the same Blog parent. The checker raises the one blocking `request_review` on pass.
...
`channel:instagram` child of the Instagram parent (changes requested first, then the earliest-due `ready` open child): caption + slide copy from the linked blog topic,
...
attached, `blog-checker` in a SEPARATE subagent, one blocking `request_review` on pass. Never Postiz, never `publish`. Print its one-line summary too.
```
...
Call Context `start_context`, Blog MCP `usage_guide` + `get_capabilities`, then follow the `blog-publisher` skill exactly:
candidates are `channel:blog` children of the Blog parent (`list_issues {parent_id: "<blog parent id>", state: "done", includeClosed: true, label: "channel:blog"}`, follow nextCursor)
whose Context state is `done` (re-read with get_issue), `due` ≤ now < `due` + 3 h, with no "Published:" update yet;
...
`channel:instagram` children of the Instagram parent whose state is `done` (re-read with get_issue), `due` ≤ now < `due` + 3 h, no "Scheduled:"/"Published:" update; dedupe guard
...
Call Context `start_context`, Blog MCP `usage_guide` + `get_capabilities`, then follow the `blog-assessment` skill exactly:
...
2. Stats, read-only, period 7d (+28d for context): `stats_summary`, `top_posts {limit: 10}`, `ai_referrals`, `search_queries {striking_distance_only: true}`,
   `indexing_status`; plus `list_issues {parent_id: "<epic id>", includeClosed: true}` for what published / bounced / waited this week.
...
5. `create_issues` ONE sub-issue under the Performance Report issue (title "Weekly report — <slug> — <ISO week>", labels tenant:<slug>, lane:performance, report:weekly, gate:none, state done),
   `attach_artifact` the report on it (docKind "report"), headline in `post_comment` on the Performance Report issue.
