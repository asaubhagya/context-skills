...
write("mcp/index.html", page("MCP servers", `<h1>MCP servers</h1><p class="mute">Two servers, one catalog each — generated from the servers' own <code>tools/list</code>. Connect the endpoint, then load the skills that wrap it.</p><div class="grid">${catalogs.map(({ m, cat }) => `<div class="card"><a class="t" href="/mcp/${m.key}">${esc(cat.server.name)}</a><div class="mute" style="font-size:14px;margin:4px 0">${esc(cat.server.purpose ?? "")}</div><div class="meta"><span>${m.toolCount} tools</span><span>contract <code>${esc(m.server.contractVersion)}</code></span><span><code>${esc(m.server.endpoint)}</code></span></div></div>`).join("")}</div>`, { alt: "/mcp/index.json" }));
